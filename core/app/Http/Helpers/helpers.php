...
function discountText($product, $general)
{
    if ($product->discount != 0) {
        if ($product->discount_type == 1 && $product->today_deals != 1) {
            $discount = $general->cur_sym . showAmount($product->discount);
        }else if($product->discount_type == 1 && $product->today_deals == 1){
            if($general->discount_type == 1){
                $discount = $general->cur_sym . showAmount($product->discount + $general->discount);
            }else{
                if($general->discount == 0){
                    $discount = $general->cur_sym . showAmount($product->discount);
                }
                else{
                    $discount = $general->cur_sym . showAmount($product->discount) .'&' . showAmount($general->discount). '%';
                }
            }
        }
        else {
